  ...
  return bindActionCreators(
    {
      ...
      getAllDonorsSortedDescendingName:
        actions.getAllDonorsSortedDescendingName,
          ...
          <tbody>
            {this.state.data.length > 0
              ? this.state.pageOfDonor.map((donor, index) => (
                  <tr key={index}>
                    <td style={{ paddingTop: "15px", paddingBottom: "25px" }}>
                      <Link
                        to={{
                          pathname: `/donor/${donor.id}/details`,
                          state: {
                            donorName: donor.donorName,
                            timesDonated: donor.totalDonatedCount
                          }
                        }}
                      >
                        {donor.donorName}
                    ...
                    <td style={{ paddingTop: "15px", paddingBottom: "25px" }}>
                      {donor.contact ? (
                        <Link
                          to={{
                            pathname: `/contacts/${donor.contact.id}/edit`
                          }}
                        >
                          {donor.contact.contactName}
                        </Link>
                      ) : (
                        "Unknown"
                      )}
